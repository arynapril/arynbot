...
exports.run = (bot, message, args, level) => {
    ...
    channelArray = ['dadJokesJail', 'hallOfFameChannel', 'welcomeMessageChannel'];
        ...
        if (args[0].indexOf(settingsArray) != -1) {
            if (!args[1]) {
                ...
                if (args[0].indexOf(booleanArray) != -1 && (args[1] !== 1 || args[1] !== 0)) return message.channel.send(`The ${args[0]} value must be be either a 0 or a 1! Please try again!`);
                if (args[0].indexOf(channelArray) != -1 && !message.mentions.channels.array()[0]) return message.channel.send(`The ${args[0]} value must be a channel mention! Please try again!`);
